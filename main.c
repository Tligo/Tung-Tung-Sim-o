...
/* Aguarda que o jogador pressione ENTER após uma mensagem de erro. */
 ...
 * Guarda o estado antes de tentar; cancela o guardado se o movimento falhar. */
...
/* Desfaz a última jogada ou informa que não há histórico disponível. */
static void processarUndo(EstadoJogo *e) {
    int ok = desfazerJogada(e);
    if (!ok) {
        printf("\n[!] Sem jogadas para desfazer!");
        aguardarEnter();
    }
}
...
static int processarOpcao(EstadoJogo *e, char opcao) {
    ...
    else if (opcao == 'u' || opcao == 'U') processarUndo(e);
    else {
        printf("\n[!] Opcao invalida!");
        aguardarEnter();
    }
